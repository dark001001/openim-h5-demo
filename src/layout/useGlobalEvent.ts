...
import { ConversationItem, GroupMemberItem, FriendApplicationItem, GroupApplicationItem, WSEvent, MessageItem } from "@/utils/open-im-sdk-wasm/types/entity";
import { MessageType, MessageReceiveOptType, SessionType } from "@/utils/open-im-sdk-wasm/types/enum";
...
export function useGlobalEvent() {
  ...
  const handleNewMessage = (newServerMsg: ExMessageItem) => {
    if (inCurrentConversation(newServerMsg)) {
      ...
      if (newServerMsg.contentType !== MessageType.TypingMessage && newServerMsg.sendID !== userStore.storeSelfInfo.userID) {
    ...
    switch (newServerMsg.sessionType) {
      case SessionType.Single:
        return (
          ...
          (newServerMsg.sendID === userStore.storeSelfInfo.userID && newServerMsg.recvID === conversationStore.storeCurrentConversation.userID)
        );
      case SessionType.Group:
      case SessionType.WorkingGroup:
        return newServerMsg.groupID === conversationStore.storeCurrentConversation.groupID;
  ...
  const newMessageNotify = async (newServerMsg: MessageItem) => {
    ...
    if (userStore.storeSelfInfo.allowBeep === BusinessAllowType.NotAllow || userStore.storeSelfInfo.globalRecvMsgOpt !== MessageReceiveOptType.Nomal) {
      return;
    }

    let cveItem = [...conversationStore.storeConversationList, ...cacheConversationList].find((conversation) => {
  ...
  const conversationChnageHandler = ({ data }: WSEvent<ConversationItem[]>) => {
    ...
    filterArr = conversationStore.storeConversationList.filter((tc) => !chids.includes(tc.conversationID));
    const idx = changes.findIndex((c) => c.conversationID === conversationStore.storeCurrentConversation.conversationID);
  ...
  const groupMemberInfoChangedHandler = ({ data }: any) => {
    ...
    if (member.groupID === conversationStore.storeCurrentMemberInGroup?.groupID && member.userID === conversationStore.storeCurrentMemberInGroup?.userID) {
