      ...
      <van-tabs v-model:active="activeTab">
        <van-tab class="overflow-y-auto" title="按好友选">
          <van-index-bar v-if="renderFriendList.indexList.length > 0" :index-list="renderFriendList.indexList">
            <template v-for="(item, idx) in renderFriendList.indexList">
              ...
              <GenericListItem
                class="bg-white"
                v-for="(friend, index) in renderFriendList.dataList[idx]"
                :key="friend.userID"
                :source="friend"
                :total="renderFriendList.dataList[idx].length"
                :index="index"
                :showCheck="showCheck"
                :disabled="disabledUserIDList.includes(friend.userID)"
        ...
        <van-tab title="按群聊选" v-if="canChooseGroupTypes.includes(chooseType)">
          <virtual-list
            ...
            class="my_scrollbar h-full overflow-y-auto overflow-x-hidden bg-white"
            :data-key="'groupID'"
            :data-sources="renderGroupList"
      ...
      beforeClose: (action) =>
        new Promise(async (resolve) => {
          if (action === "confirm") {
            ...
            const message = (await IMSDK.createCardMessage(item)).data;
  ...
  switch (chooseType) {
    ...
    case ContactChooseEnum.InviteGroup:
      IMSDK.inviteUserToGroup({
        ...
        .then(() => feedbackToast({ message: "邀请成功！" }))
        .catch(() => feedbackToast({ message: "邀请失败！" }))
        .finally(() => router.back());
      break;
    default:
      break;
