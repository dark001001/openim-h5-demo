...
const useStore = defineStore("contact", {
  ...
  actions: {
    ...
    updateFriendList(item: FriendUserItem, isRemove = false) {
      const idx = this.friendList.findIndex((friend: FriendUserItem) => friend.userID === item.userID);
      if (idx !== -1) {
        if (isRemove) {
    ...
    updateGroupList(item: GroupItem, isRemove = false) {
      const idx = this.groupList.findIndex((group: GroupItem) => group.groupID === item.groupID);
      if (idx !== -1) {
        if (isRemove) {
    ...
    updateBlackList(item: BlackUserItem, isRemove = false) {
      const idx = this.blackList.findIndex((user: BlackUserItem) => user.userID === item.userID);
      if (idx !== -1) {
        if (isRemove) {
    ...
    updateRecvFriendApplicationList(item: FriendApplicationItem) {
      const idx = this.recvFriendApplicationList.findIndex((application: FriendApplicationItem) => application.fromUserID === item.fromUserID);
      if (idx !== -1) {
        this.recvFriendApplicationList[idx] = { ...item };
      }
    },
    pushNewRecvFriendApplication(item: FriendApplicationItem) {
      const idx = this.recvFriendApplicationList.findIndex((application) => application.fromUserID === item.fromUserID);
      if (idx > -1) {
        this.recvFriendApplicationList[idx] = { ...item };
      } else {
    ...
    updateSendFriendApplicationList(item: FriendApplicationItem) {
      const idx = this.sendFriendApplicationList.findIndex((application: FriendApplicationItem) => application.toUserID === item.toUserID);
      if (idx !== -1) {
        this.sendFriendApplicationList[idx] = { ...item };
      }
    },
    pushNewSendFriendApplication(item: FriendApplicationItem) {
      const idx = this.sendFriendApplicationList.findIndex((application) => application.toUserID === item.toUserID);
      if (idx > -1) {
        this.sendFriendApplicationList[idx] = { ...item };
      } else {
    ...
    updateRecvGroupApplicationList(item: GroupApplicationItem) {
      const idx = this.recvGroupApplicationList.findIndex((application: GroupApplicationItem) => application.userID === item.userID);
      if (idx !== -1) {
        this.recvGroupApplicationList[idx] = { ...item };
      }
    },
    pushNewRecvGroupApplication(item: GroupApplicationItem) {
      const idx = this.recvGroupApplicationList.findIndex((application) => application.userID === item.userID);
      if (idx > -1) {
        this.recvGroupApplicationList[idx] = { ...item };
      } else {
    ...
    updateSendGroupApplicationList(item: GroupApplicationItem) {
      const idx = this.sendGroupApplicationList.findIndex((application: GroupApplicationItem) => application.groupID === item.groupID);
      if (idx !== -1) {
        this.sendGroupApplicationList[idx] = { ...item };
      }
    },
    pushNewSendGroupApplication(item: GroupApplicationItem) {
      const idx = this.sendGroupApplicationList.findIndex((application) => application.groupID === item.groupID);
      if (idx > -1) {
        this.sendGroupApplicationList[idx] = { ...item };
      } else {
    ...
    updateUserCardMemberInfo(item: GroupMemberItem) {
      if (item.userID !== this.userCardData.groupMemberInfo?.userID || item.groupID !== this.userCardData.groupMemberInfo?.groupID) return;
